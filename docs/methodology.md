...
*   **Escalation Email Notifications:** When handoff is required, the backend sends owner alerts through Resend transactional email first, SMTP second, and console simulation in local development. The customer remains inside the chatbot while the public chat interface polls for representative replies sent from the dashboard.
