...
* **Smart Hybrid Escalation and Real-Time Alerting**: An asynchronous out-of-band notification engine (Email and WhatsApp deep linking) combined with an in-chat lead capture mechanism that alerts merchant owners immediately when customer queries require human intervention, preventing customer bypass and eliminating response bottlenecks.
...
5. Implement a dual-layer confidence score thresholding mechanism ($\tau=0.50$) and human handoff classification to trigger low-confidence fallback responses, capture customer contact details, and dispatch real-time out-of-band email and WhatsApp alerts to business owners.
