...
4.  **Database Seeding Script:** Added a script (`seed.py`) populating the database with a sample Ghanaian SME (electronics shop "Kojo's Tech Hub" located at Adum, Kumasi) with products, services, and FAQs.
...
```
Seeding database...
Database successfully seeded with Kojo's Tech Hub sample data!
```
