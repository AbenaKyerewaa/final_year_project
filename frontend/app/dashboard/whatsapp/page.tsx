...
export default function WhatsAppDashboard() {
  ...
  useEffect(() => {
    async function fetchConfig() {
      if (!token) return;
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/webhooks/whatsapp/config`, {
          headers: {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5 transition-colors duration-300">
        ...
        <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-500/20 px-3 py-1.5 rounded-full text-emerald-600 dark:text-emerald-400 text-xs font-semibold shadow-sm dark:shadow-none transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/40 backdrop-blur-xl p-6 space-y-4 shadow-sm dark:shadow-none transition-colors duration-300">
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-400 uppercase tracking-wider">Callback URL</span>
                <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5">
                  <code className="text-xs text-slate-700 dark:text-slate-300 font-mono flex-grow truncate">{config.backendWebhookUrl}</code>
                  <button
                    onClick={() => copyToClipboard(config.backendWebhookUrl, 'url')}
                    className="p-1.5 rounded hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white transition duration-200"
                  >
                    {copiedField === 'url' ? (
                      <span className="text-[10px] text-emerald-500 dark:text-emerald-400 font-bold uppercase">Copied!</span>
                    ) : (
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {/* Verify Token */}
              <div className="flex flex-col gap-1.5">
                <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-400 uppercase tracking-wider">Verification Token</span>
                <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5">
                  <code className="text-xs text-slate-700 dark:text-slate-300 font-mono flex-grow truncate">{config.verifyToken}</code>
                  <button
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/40 backdrop-blur-xl p-6 space-y-4 shadow-sm dark:shadow-none transition-colors duration-300">
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Active Configuration</h3>
            
            <div className="space-y-3.5 text-sm">
              <div className="border-b border-slate-100 dark:border-slate-900 pb-2">
                <span className="text-xs text-slate-400 dark:text-slate-400 block">WhatsApp Number ID</span>
                <span className="text-slate-700 dark:text-slate-300 font-mono text-xs">{config.phoneNumberId}</span>
              </div>

              <div className="border-b border-slate-100 dark:border-slate-900 pb-2">
                <span className="text-xs text-slate-400 dark:text-slate-400 block">Business Number Map</span>
                <span className="text-slate-700 dark:text-slate-300 font-semibold">{config.businessNumber}</span>
              </div>

              <div>
                <span className="text-xs text-slate-400 dark:text-slate-400 block">WhatsApp Credentials Status</span>
                <span className="text-amber-600 dark:text-amber-400 font-semibold text-xs flex items-center gap-1.5 mt-1">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800/40 bg-white dark:bg-slate-950/20 p-6 space-y-3 shadow-sm dark:shadow-none transition-colors duration-300">
            <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Meta Integration Steps</h3>
            <ul className="space-y-2.5 text-xs text-slate-600 dark:text-slate-400">
              ...
              <li className="flex gap-2">
                ...
                Set <code className="text-slate-600 dark:text-slate-300 font-mono bg-slate-50 dark:bg-slate-900 px-1 py-0.5 rounded">WHATSAPP_MODE=cloud_api</code> in your <code className="text-slate-600 dark:text-slate-300 font-mono bg-slate-50 dark:bg-slate-900 px-1 py-0.5 rounded">backend/.env</code> file.
              </li>
              <li className="flex gap-2">
