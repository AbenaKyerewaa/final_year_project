...
export default function EditBusiness({ params }: PageProps) {
  ...
  // Load existing business metadata on mount
  useEffect(() => {
    async function loadBusinessData() {
      if (!token || !id) return;
      try {
        const data = await getBusiness(id, token);
        ...
        setError(err.message || "Failed to load business profile data.");
      } finally {
