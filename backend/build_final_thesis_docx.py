...
def build_thesis_docx():
    ...
    add_num_item("5.", "", "Implement a dual-layer confidence score thresholding mechanism (tau = 0.50) to trigger low-confidence fallback responses, capture customer contact details, and create real-time human escalation tickets with Resend email alerts and WhatsApp reply links.")
    ...
    add_bullet("Conversational Logs & Escalations Viewer:", "Displays active customer chat history across sessions and flags inquiries escalated to human representatives, including submitted customer phone or WhatsApp contact details.")
    ...
    add_bullet("In-Chat Contact Capture Flow:", "Upon escalation, the web chat widget displays an embedded contact capture card. Submitting a phone or WhatsApp number triggers POST /chat/{business_id}/contact, persists the contact to the session, and dispatches an updated owner alert containing the customer's submitted name and phone or WhatsApp number.")
    add_bullet("Dashboard Indicators and One-Click WhatsApp Reply:", "The merchant dashboard displays real-time pending counters and an urgent home banner. Within the session detail view, merchants can initiate a one-click conversation on WhatsApp or submit a direct reply as an authenticated representative via POST /chat-sessions/{session_id}/reply, which auto-resolves the escalation ticket.")
    ...
    add_bullet("Customer interactions across web chat and WhatsApp can be automated securely", "while preserving human oversight through automated escalation handoffs, Resend email alerts, and WhatsApp follow-up links.")
